...
describe('NPM retrieval functions', () => {
  describe('getNpmData()', () => {
    ...
    it('throws for unexpected server response', async () => {
      try {
        nock('https://registry.npmjs.org:443').get('/nothingburger42').reply(429, { message: 'Quota limit reached' });
        await expect(async () => await getNPMPackage('nothingburger42')).rejects.toThrowErrorMatchingInlineSnapshot(`
  ...
  describe('getDependenciesDeep()', () => {
    ...
    it('throws for non-existent dependency version', async () => {
      ...
      nock('https://registry.npmjs.org:443').get('/imaginarychild').reply(200, childPackage);
      try {
        await expect(async () => {
